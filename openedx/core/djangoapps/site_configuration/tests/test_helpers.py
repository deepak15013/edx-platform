...
class TestHelpers(TestCase):
    ...
    @with_site_configuration(configuration=test_config1)
    def test_get_value(self):
        """
        Test that get_value returns correct value for any given key.
        """
        # Make sure entry is saved and retrieved correctly
        self.assertEqual(configuration_helpers.get_value("university"), test_config1['university'])
        self.assertEqual(configuration_helpers.get_value("platform_name"), test_config1['platform_name'])
        self.assertEqual(configuration_helpers.get_value("SITE_NAME"), test_config1['SITE_NAME'])
        self.assertEqual(configuration_helpers.get_value("course_org_filter"), test_config1['course_org_filter'])
        self.assertEqual(configuration_helpers.get_value("css_overrides_file"), test_config1['css_overrides_file'])
        self.assertEqual(configuration_helpers.get_value("ENABLE_MKTG_SITE"), test_config1['ENABLE_MKTG_SITE'])
        self.assertEqual(configuration_helpers.get_value("favicon_path"), test_config1['favicon_path'])
        self.assertEqual(
            configuration_helpers.get_value("ENABLE_THIRD_PARTY_AUTH"),
            test_config1['ENABLE_THIRD_PARTY_AUTH'],
        )
        self.assertEqual(
            configuration_helpers.get_value("course_about_show_social_links"),
            test_config1['course_about_show_social_links'],
        )
    ...
    def test_get_dict(self):
        ...
        self.assertItemsEqual(
            ...
            test_config1['REGISTRATION_EXTRA_FIELDS'],
        )

        default = {"test1": 123, "first_name": "Test"}
        expected = default
        expected.update(test_config1['REGISTRATION_EXTRA_FIELDS'])
    ...
    def test_is_site_configuration_enabled(self):
        ...
        """
        with with_site_configuration_context(configuration=test_config1):
    ...
    def test_get_all_orgs(self):
        ...
        """
        test_orgs = [test_config1['course_org_filter']]
        with with_site_configuration_context(configuration=test_config1):
            self.assertItemsEqual(
